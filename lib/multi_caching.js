...
var multiCaching = function (caches, options) {
     ...
     *
     * @param {string} key
     * @param {object} [options] - options passed to get function
     * @param {function} cb
     */
    self.getAndPassUp = function (key, options, cb) {
        getFromHighestPriorityCache(key, options, function (err, result, index) {
            if (err) {
                return cb(err);
            }
            ...
            if (index) {
                ...
                async.each(cachesToUpdate, function (cache, next) {
                    ...
                    if (_isCacheableValue(result)) {
                        ...
                        cache.set(key, options, result, next);
                    }
                });
            }
        });
     ...
     * @param {object} [options] - options passed to set/get function
     * @param {function} cb
     */
        ...
        getFromHighestPriorityCache(key, options, function (err, result, index) {
            if (err) {
                ...
                domain
                    ...
                    .bind(work)(function (err, data, meta) {
                        if (err) {
                            return cb(err);
                        }
